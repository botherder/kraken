...
linux: check-env
	...
	@go build --ldflags '-s -w -X main.URLBaseDomain=$(BACKEND)' -tags yara_static -o $(BUILD_FOLDER)/linux/kraken
...
darwin: check-env
	...
	@go build --ldflags '-s -w -X main.URLBaseDomain=$(BACKEND)' -o $(BUILD_FOLDER)/darwin/kraken
...
windows: check-env
	...
	@$(FLAGS_WINDOWS) go build --ldflags '-s -w -extldflags "-static" -X main.URLBaseDomain=$(BACKEND)' -o $(BUILD_FOLDER)/windows/kraken.exe
